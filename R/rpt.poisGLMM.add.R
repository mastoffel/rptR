...
rpt.poisGLMM.add_ <- function(data, y, groups, CI = 0.95, prior = NULL, verbose = FALSE, 
    ...
    P.org <- NA
    # return of results
    res <- list(call = match.call(), datatype = "count", method = "MCMC", CI = CI, R.link = R.link, 
        se.link = se.link, CI.link = CI.link, P.link = P.link, R.org = R.org, se.org = se.org, 
        CI.org = CI.org, P.org = P.org, R.post = list(R.link = as.vector(postR.link), 
