...
print.rpt <- function(x, ...) {
    ...
    # if(x$datatype=='Gaussian' & length(x$P)==1 & length(x$R)>1) { print('h3')
    # cat('\n', 'Repeatability calculation using the ', x$method, ' method', '\n\n')
    # for(i in 1: length(x$R)) { cat('Repeatability for ', names(x$R)[i], '\n', 'R = ',
    # round(x$R[i],3), '\n', 'SE = ', round(x$se[i],3), '\n', 'CI = [',
    # round(x$CI.R[i,1],3), ', ', round(x$CI.R[i,2],3), ']', '\n', 'P = ',
    # signif(x$P[i], 3), '\n\n', sep='') } } rpt.remlLMM.adj with >1 groups factor
    if (x$datatype == "Gaussian" & length(x$P) > 1 & length(x$R) > 1) {
        cat("\n", "Repeatability calculation using the ", x$method, " method", "\n\n")
        for (i in 1:length(x$R)) {
            cat("Repeatability for ", names(x$R)[i], "\n", "R  = ", round(x$R[i], 3), 
                "\n", "SE = ", round(x$se[i], 3), "\n", "CI = [", round(x$CI.R[i, 1], 
                  ...
                  1], 3), " [", attr(x$P, "names")[1], "]", "\n", "     ", signif(x$P[i, 
                  2], 3), " [", attr(x$P, "names")[2], "]", "\n\n", sep = "")
        }
    }
